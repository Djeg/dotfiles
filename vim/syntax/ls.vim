...
""" Hindley Milner Typing support (with purescript syntax)
syn include @Purescript syntax/purescript.vim
syn region hindleyMilnerComment start=+/\*\*\*+ end=+\*/+ keepend fold contains=@Purescript
"" Start HM fold marker"{{{
syn match hindleyMilnerOneLineComment "##.*" contains=@Purescript oneline keepend
"}}}
""" End of Hindley Milner
...
syn region typescriptStringE start=+'''+ end=+'''+  contains=typescriptInterpolation,typescriptSpecial,@htmlPreproc extend
syn region typescriptStringF start=+"""+ end=+"""+  contains=typescriptInterpolation,typescriptSpecial,@htmlPreproc extend
...
syn region graphqlBlock start=+\%(gql\|graphql\|GraphQL\|GQL\)[ ]\+'''+ end=+'''+ keepend fold contains=@GraphQLSyntax
""" End of graphql support
...
syn region styledComponentBlock start=+styled\.[a-zA-Z0-1 ]\+'''+ end=+'''+ keepend fold contains=@Css
""" End of styled component support
...
syntax keyword typescriptReserved constructor declare as interface module abstract enum int short export interface static byte extends long super char final native synchronized class float package throws goto private transient debugger implements protected volatile double import require public type namespace from get set
"}}}
...
if version >= 508 || !exists("did_typescript_syn_inits")
  ...
  HiLinkT hindleyMilnerOneLineComment Comment
  HiLinkT hindleyMilnerOneLineComment2 Comment
